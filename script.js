...
class Library {
    ...
    removeBook(title){
        ...
        if(index === -1){
           ...
           return this.#books.splice(title)
        }
...
// console.log(myLibrary.hasBook('Книга'));

// myLibrary.addBook('Книга 4');
// console.log(myLibrary.hasBook('Книга 4'));
console.log(myLibrary.removeBook('Книга 2'));
...
const errorMes = document.querySelector('.error_msg');

initialData.forEach(element => {
    const productName = document.createElement('h3');
    productName.textContent = element.product
    review.appendChild(productName);
    element.reviews.forEach(reviewInit => {
        const reviewContent = document.createElement('p');
        reviewContent.textContent = reviewInit.text;
...
button.addEventListener('click', function () {
    try {
        if(inputText.value.length < 20 || inputText.value.length > 500){
            throw new Error('Несоответствующая длинна ')
        }
        const reviewElem = document.createElement('p');
        reviewElem.textContent = inputText.value;
        review.appendChild(reviewElem);
        errorMes.textContent = '';
        
    } catch (error) {
        errorMes.textContent = error.message;
    }
